...
```
Still weird. Oh wait, I am comparing a model with random slope and one without, maybe that isn't good.
```{r}
logLik(mod_5b)* -2
logLik(mod_6b)* -2
717437.7 - 719577.9
...
anova(mod_5b, mod_6b, test = "LRT")

glance(mod_2b)
glance(mod_7b)

mod_6c <- lmer(CRMCYPERC ~ gndctr_ndvi_cbg_adj + gndctr_MedHHinc000 + gndctr_disad + gndctr_diver + gndctr_U18 + gndctr_logpopden + gndctr_MetroGDP + gndctr_Police + gndctr_rt_violent+clust4 + (gndctr_ndvi_cbg_adj|city_st), data = df_na, REML = FALSE)
anova(mod_5b, mod_6c)
anova(mod_4b, mod_6b)
```
```{r}
